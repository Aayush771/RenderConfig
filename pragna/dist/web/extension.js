...
async function fetchDashboard(fileName, content, projectName) {
    try {
        ...
        const panel = vscode.window.createWebviewPanel("dashboardPanel", // Identifies the type of the webview. Used internally
        ...
        vscode.window.showInformationMessage("Dashboard opened in a webview panel.");
    }
    catch (error) {
        console.error("Error fetching dashboard:", error.message);
        vscode.window.showErrorMessage("Error fetching dashboard. Check console for details.");
    }
}
function getWebViewContent(responseData) {
  ...
  --list-item-background-color: #121212; /* Darker grey highlight */
    ...
    <body class="light-mode">
      <div class="container">
        ...
        .map((suggestion) => `<li>Line ${suggestion.lineNumber}: ${suggestion.text}</li>`)
        .join("")}
          </ul>
        </div>
        ...
        <div class="dashboard-item">
          <h3>CheckStyle</h3>
          <ul>
            <li>Code Reusability (${responseData.checkStyle.checkboxes[0] ? "Passed" : "Violated"})</li>
            <li>Secrets of constant and keys (${responseData.checkStyle.checkboxes[1] ? "Passed" : "Violated"})</li>
            <li>Code Structure (pre-provided by us)</li>
            <li>Naming Conventions (${responseData.checkStyle.checkboxes[3] ? "Passed" : "Violated"})</li>
            <li>Industrial comments naming Structure (${responseData.checkStyle.checkboxes[4] ? "Passed" : "Violated"})</li>
        ...
        <div class="dashboard-item">
          ...
          <p><strong>Scope for Improvement:</strong> ${responseData.summary.scopeForImprovement}</p>
        </div>

        <div class="dashboard-item">
          <h3>Occurrence Line Numbers</h3>
          <p>${responseData.checkStyle.occuranceLineNumbers
        .map((lines) => lines.length > 0 ? lines.join(", ") : "No occurrences")
        .join("<br>")}</p>
        </div>
      </div>
...
function activate(context) {
    console.log('Congratulations, your extension "pragna" is now active in the web extension host!');
    const helloCommand = vscode.commands.registerCommand("pragna.helloWorld", () => {
        vscode.window.showInformationMessage("Hello Robert from pragna in a web extension host!");
    });
    const scanCommand = vscode.commands.registerCommand("pragna.scan", async () => {
